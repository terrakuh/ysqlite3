...
{
	if (argc != 2) {
		throw std::system_error{ SQLite3_code::generic, "requires 2 arguments" };
	}
	...
	if (!pattern) {
		...
		if (!str) {
			throw std::system_error{ SQLite3_code::generic, "bad regex" };
		}
		// compile
		try {
			pattern =
			...
			throw std::system_error{ SQLite3_code::generic, e.what() };
		}
		// cache for later use
